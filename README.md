...
In this tutorial, we will be using AWS CodePipeline to deploy a serverless and add Cloudguard workload as a step in the build and deploy phases of the CI/CD pipeline. In this Tutorial we will show how to build a simple Lambda function and its Cloud Formation stack using AWS SAM(Serverless Application Model) CLI and then create a CICD pipeline with AWS CodePipeline to build, secure and deploy that Lambda function. We will integration the Cloudguard Workload security to AWS CodePipeline using cli plugin to perform Governance, Deep code analysis and Runtime application security.
...
Note: I will be using MacOS in this tutorial and feel free to use any OS of your choice.

The AWS CodePipeline create a continuous integration and deployment pipeline to build and deploy a Lambda function. we will using the SAM cli to generate the source code for the Lambda and needed templates for the AWS Codepipeline as follow. In a CICD pipeline, there are 4 main stages from build to Runtime. Additional stages for staging and testing can be added as required by the Software development lifecycle.
...
Next steps will be to intialize your project directory with Git, commit and push all the files to your Repository of choice. 
